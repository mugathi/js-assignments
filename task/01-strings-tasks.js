...
function extractNameFromTemplate(value) {
    ...
    return values[1]+' '+values[2].substr(0,values[2].length-1);
}
...
function encodeToRot13(str) {
    str = Array.from(str).map((i)=>{
        let code = i.charCodeAt(0);
        if(code<78&&code>=65||code<110&&code>=97){
            i= String.fromCharCode(code+13);
...
function getCardId(value) {
    let first = value.substring(0,value.length-1);
    let second = value.substring(value.length-1);
    switch(first){
